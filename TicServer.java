...
public class TicServer implements ActionListener
{
	...
	private final String url = "jdbc:hsqldb:file:saveServerGames";
	private Connection connection = null;
	
	public TicServer() throws UnknownHostException, IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException
	{
		myTurn = false;
	...
	{
		frame = new JFrame("TicTacToe - Serwer");
		JPanel mainPanel = new JPanel();
		JPanel gamePanel = new JPanel();
		JPanel optionPanel = new JPanel();
		JLabel label = new JLabel("Grasz Krzyzykami!");
	...
	public boolean checkIfWin(String position)
	{
		...
		if(tab[x][y].getText().equals("X"))
		{
			...
			while(temp<20)
			{
				if(tab[temp][y].getText().equals("X"))
				{
					size++;
					temp++;
				}
			...
			while(temp>-1)
			{
				if(tab[x][temp].getText().equals("X"))
				{
					size_pion++;
					temp--;
				}
			...
			{
				if(tab[temp_x][temp_y].getText().equals("X"))
				{
					skos_1++;
					temp_x++;
					temp_y++;
